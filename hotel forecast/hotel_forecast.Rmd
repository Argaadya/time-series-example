...
Below is the best configuration for each series. To interpret the MAE values, we need to consider the range of the data, shown as the standar deviation of the data.
...
metric_crossing %>% 
  ...
  select_if(~is.list(.) == F) %>% 
  select(-reverse_name) %>% 
  select(hotel, market_segment, mae, mean, std_dev, everything()) %>% 
  arrange(mae)
  
```

Since we don't have much data, only two years of transactions, the model performance may not perform so well. However, judging from the MAE value, the performance is quite acceptable, with most of the error values are less than the value of one standard deviation. The best model from the series is the ARIMA model for City Hotel and Offline TA segment. 

If you wish to get the model or the forecasting result, just re-run the former steps using only the best adjustment. Here is one of the forecasting result for the Online TA segments. The actual demand is still inside the forecasting interval value.
...
data_test <- test_list$data[[6]] %>% msts(start = 28.25, seasonal.periods =  c(7, 7*4))
...
model_best[[6]] %>%
  ...
  scale_x_continuous(limits = c(25, 30), 
                     labels = as.Date.numeric(25:30*28-28, origin = range(hotel$arrival_date)[1])) +
...
```{r}
best_adjust %>% 
  select(hotel, market_segment, model_name) %>% 
  bind_cols( 
...
Based on the result, all of our model didn't fulfill the normality assumption for the residuals. This suggest that we can improve the model further in order to get better performance. 
